...
const MessagesList = (companionData: CompanionData | undefined, messages: Messages) => {
    ...
    const send_updated_message = (messageId: number) => {
        ...
        fetch('/api/editMessage', {
          ...
          .catch((error) => {
            console.error('Error while updating a message: ', error);
          });
      };
            ...
            {messages.map((message: Message, index: number) => message.ai ? 
                <>
                    <div className="chat chat-start pl-2">
                        ...
                        {editMode === message.id ? (
                            <input type="text" className="chat-bubble input input-bordered input-primary w-full max-w-xs" style={{ whiteSpace: "pre-line" }} value={editedMessageText}
                             onChange={(e) => {setEditedMessageText(e.target.value)}} onKeyDown={(e) => {
                                if (e.key === "Escape") {
                                    end_edit();
                                  } else if (e.key === "Enter") {
                            ...
                            <div className="chat-bubble" style={{ whiteSpace: "pre-line" }} dangerouslySetInnerHTML={{ __html: (index === messages.length - 1 && index != 0 && regeneratingMessage ? "Regenerating message..." : safe_eval(message.text)) }}></div>
                        )}
                        </div>
                        <div className="flex space-x-2 pl-14">
                            {index === messages.length - 1 && index != 0 && !regeneratingMessage && (
                                ...
                                .catch((error) => {
                                  ...
                                  <div className="tooltip tooltip-bottom" data-tip="Regenerate message"> 
                            ...
                            <>
                                <div className="chat-footer tiny-text opacity-50 cursor-pointer" onClick={() => start_edit(message.id, message.text)}>
                                    <div className="tooltip tooltip-bottom" data-tip="Edit message">
                                ...
                                <div className="tooltip tooltip-bottom" data-tip="Remove message">
                ...
                <>
                    <div className="chat chat-end pr-2">
                        ...
                        {editMode === message.id ? (
                            <input type="text" className="chat-bubble input input-bordered input-primary w-full max-w-xs" style={{ whiteSpace: "pre-line" }} value={editedMessageText} onChange={(e) => {setEditedMessageText(e.target.value)}} onKeyDown={(e) => {
                                if (e.key === "Escape") {
                                    end_edit();
                                  } else if (e.key === "Enter") {
                                      send_updated_message(message.id);
...
const ChatWindow = (companionData: CompanionData | undefined) => {
    ...
    return (
        <div className="chat-container mockup-window border bg-base-300 h-3/5 overflow-y-scroll flex flex-col-reverse">
